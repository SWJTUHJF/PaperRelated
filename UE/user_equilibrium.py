...
class LINK:
    ...
    def get_specific_cost(self, temp):
...
class FW:
    def __init__(self, name, algorithm, BC, FWC):
        self.name: str = name
    ...
    def convergence(self):
        numerator = sum([link.flow * link.cost for link in self.LINKS])
        denominator = sum([link.auxiliary_flow * link.cost for link in self.LINKS])
    ...
    def total_system_travel_time(self):
        total = sum([link.cost * link.flow for link in self.LINKS])
...
if __name__ == "__main__":
    ...
    net = FW(name='Chicago-Sketch', algorithm='LS', BC=0.001, FWC=0.001)
    print("*****Conducting Frank-Wolfe algorithm*****")
    net.conduct_FW()
    net.log_out()
